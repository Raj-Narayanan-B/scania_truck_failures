...
def s3_bucket():
    ...
    if request.method == "GET":
        ...
        s3_files_list_ = [file_name for file_name in s3_files_list if not file_name.startswith("Prediction")]
        print(s3_files_list_)
        return render_template('S3_bucket.html', s3_file_list=s3_files_list_)
    else:
        try:
            selected_file = request.form.get('selected_file')
            s3_obj.file_lineage_tracker()
            file_lineage = load_yaml(s3_obj.get_data_path_config().data_from_s3)
            pprint(file_lineage,compact=True)
